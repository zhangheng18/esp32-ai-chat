...
class ST7789(framebuf.FrameBuffer):
    ...
    def rotate(self, rotate):
        ...
        if self.height == 320 or self.width == 320:
            table = [(320, 240, 0, 0), (240, 320, 0, 0)]
        elif self.width == 320:
            table = WIDTH_320
        elif self.width == 240:
            table = WIDTH_240
        ...
        super().__init__(
            self.buffer, self.width, self.height, framebuf.RGB565, self.width
        )
    ...
    def _set_columns(self, start, end):
        ...
        if start <= end <= self.width:
            self._write(
                ST7789_CASET, _encode_pos(start + self.x_start, end + self.x_start)
            )
    ...
    def _set_rows(self, start, end):
        ...
        if start <= end <= self.height:
            self._write(
                ST7789_RASET, _encode_pos(start + self.y_start, end + self.y_start)
            )
    ...
    def rgb(self, enable: bool):
        ...
        self._write(
            ST7789_MADCTL,
            bytes([ROTATIONS[self._rotate] | (0x00 if self._rgb else 0x08)]),
        )
