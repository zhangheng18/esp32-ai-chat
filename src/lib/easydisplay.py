...
class EasyDisplay:
    READ_SIZE = 128  # Limit the picture read size to prevent memory errors in low-performance development boards
    CACHE_SIZE = 500
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._buffer = hasattr(
            display, 'buffer'
        )  # buffer: 驱动是否使用了帧缓冲区，False（SPI 直接驱动） / True（Framebuffer）
        self._font = None
        self._key = key
        self._show = show
        self._clear = clear
    ...
    @staticmethod
    def rgb565_color(r: int, g: int, b: int) -> int:
        """
    ...
    @staticmethod
    def _hlsb_font_size(
        bytearray_data: bytearray, new_size: int, old_size: int
    ) -> bytearray:
        """
        ...
        r = range(
            new_size
        )  # Preload functions to avoid repeated execution and improve efficiency
        ...
        for _col in r:
            for _row in r:
                ...
                _t[_new_index] = _t[_new_index] | (
                    (bytearray_data[_old_index >> 3] >> (7 - _old_index % 8) & 1)
                    << (7 - _row % 8)
                )
        return _t
    ...
    @micropython.native
    def get_cached_fbuf(self, char: str, font_size: int):
        """从缓存中获取字符的 FrameBuffer."""
        cache_key = (char, font_size)
        if cache_key in self.char_cache:
            return self.char_cache[cache_key]
        return None

    def cache_fbuf(self, char, font_size, fbuf):
        """缓存字符的 FrameBuffer."""
        cache_key = (char, font_size)
        if len(self.char_cache) >= self.CACHE_SIZE:
            # 缓存满了，移除最早添加的项目
            self.char_cache.popitem()
        self.char_cache[cache_key] = fbuf
    ...
    def text(
        ...
        **kwargs
        ...
        if color_type == "MONO":
            palette = FrameBuffer(
                bytearray(1), 2, 1, MONO_HLSB
            )  # MONO pixels occupy 1 byte for every 8 pixels
        elif color_type == "RGB565":
            palette = FrameBuffer(
                bytearray(4), 2, 1, RGB565
            )  # RGB565 pixels occupy 2 bytes for every 1 pixel
        else:
        ...
        for char in s:
            if auto_wrap and (
                (x + font_offset > dp.width and ord(char) < 128 and half_char)
                or (x + font_size > dp.width and (not half_char or ord(char) > 128))
            ):
            ...
            # 尝试从缓存中获取fbuf
            fbuf = self.get_cached_fbuf(char, font_size)
            if fbuf is None:
                # 如果缓存中没有，则生成新的fbuf
                byte_data = self.get_bitmap(char)
                byte_data = bytearray(byte_data)
                if font_size != self.font_size:
                    byte_data = self._hlsb_font_size(
                        byte_data, font_size, self.font_size
                    )

                fbuf = FrameBuffer(byte_data, font_size, font_size, MONO_HLSB)
                self.cache_fbuf(char, font_size, fbuf)
            ...
            if self._buffer:  # FrameBuffer Driven
                ...
                if color_type == "RGB565":
                    n_fbuf = FrameBuffer(
                        bytearray(font_size * font_size * 2),
                        font_size,
                        font_size,
                        RGB565,
                    )
                    n_fbuf.blit(
                        fbuf, 0, 0, key, palette
                    )  # Render black and white pixels to color
    ...
    def pbm(
        ...
        if isinstance(file, BytesIO):
            ...
            close = True
        try:
            file_format = func.readline()  # 获取文件格式
            _width, _height = [
                int(value) for value in func.readline().split()
            ]  # 获取图片的宽度和高度
            f_read = func.read
            if file_format == b"P4\n":  # P4 位图 二进制
                # 颜色反转
                ...
                if self._buffer:  # Framebuffer 模式
                    ...
                    if color_type == "RGB565":
                        data_fbuf = FrameBuffer(
                            bytearray(buffer_size * 16), width, 1, RGB565
                        )
                    elif color_type == "MONO":
                        data_fbuf = FrameBuffer(
                            bytearray(buffer_size), width, 1, MONO_HLSB
                        )  # Not tested
                    else:
                        raise ValueError(
                            "Unsupported color_type: {}".format(color_type)
                        )
                    ...
                    while data:
                        ...
                        data_fbuf_blit(
                            fbuf, 0, 0, key, palette
                        )  # Render MONO pixels into RGB565 pixels
                        len_data = len(data)
                        if (
                            len_data < buffer_size
                        ):  # Limit the data sent to no more than the Buffer size, so as to avoid data overflow and affect the display
                            if color_type == "RGB565":
    ...
    def bmp(
        ...
        with func as f:
            ...
            if f_read(2) == b'BM':  # 检查文件头
                dummy = f_read(
                    8
                )  # 文件大小占四个字节，文件作者占四个字节，file size(4), creator bytes(4)
                ...
                if int_fb(f_read(2), 'little') == 1:  # 色彩平面数 planes must be 1
                    ...
                    # 转换时只支持二十四位彩色，不压缩的图像
                    if (
                        depth == 24 and int_fb(f_read(4), 'little') == 0
                        ...
                        raise TypeError(
                            "Unsupported file type: only 24-bit uncompressed BMP images are supported."
                        )
            else:
